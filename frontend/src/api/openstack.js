// src/api/openstack.js — All OpenStack API calls
...
export const identity = {
  ...
  project: (id) => GET(`/identity/projects/${id}`),
  users: () => GET("/identity/users"),
  user: (id) => GET(`/identity/users/${id}`),
...
export const compute = {
  ...
  aggregates: () => GET("/compute/aggregates"),
...
export const network = {
  ...
  ports: (netId) =>
    GET(`/network/ports${netId ? "?network_id=" + netId : ""}`),
  ...
  addRouterInterface: (id, body) =>
    POST(`/network/routers/${id}/interface`, body),
  removeRouterInterface: (id, body) =>
    POST(`/network/routers/${id}/interface/remove`, body),
  ...
  sgRules: (sgId) =>
    GET(
      `/network/security-group-rules${
        sgId ? "?security_group_id=" + sgId : ""
      }`
    ),
  createSgRule: (body) =>
    POST("/network/security-group-rules", body),
  deleteSgRule: (id) =>
    DEL(`/network/security-group-rules/${id}`),
...
export const image = {
  ...
  upload: (body) => POST("/images/upload", body),
};
...
export const volume = {
  ...
  extend: (id, body) =>
    POST(`/volumes/${id}/action`, body),
  snapshots: () => GET("/volumes/snapshots"),
  createSnapshot: (body) =>
    POST("/volumes/snapshots", body),
  deleteSnapshot: (id) =>
    DEL(`/volumes/snapshots/${id}`),
  volumeTypes: () => GET("/volumes/types"),
  quotas: (pid) =>
    GET(`/volumes/quota?project_id=${pid || ""}`),
};
